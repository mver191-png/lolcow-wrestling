...
extends Node
## Cosmetic post-process. Styles below are fictional ring identities, not claims
## about the real people represented by the roster.
const IK=preload("res://scripts/fighter/contact_ik.gd")
const STYLE={
	"tophiachu":{"stance":.85,"tempo":.82,"twist":.72,"guard":.82},
	"novaonline":{"stance":.92,"tempo":.90,"twist":.88,"guard":.70},
	"cyraxx":{"stance":.48,"tempo":1.34,"twist":1.18,"guard":.92},
	"candy_rooks":{"stance":.78,"tempo":.94,"twist":.84,"guard":.78},
	"andy_ditch":{"stance":1.0,"tempo":.72,"twist":.58,"guard":.90},
	"jupiter_the_hybrid":{"stance":.60,"tempo":1.12,"twist":1.12,"guard":.86},
	"anacondasin":{"stance":.58,"tempo":1.02,"twist":1.04,"guard":.96},
	"daniel_larson":{"stance":.44,"tempo":1.24,"twist":1.08,"guard":.72}}
...
func _signature(style:Dictionary,mobility:float,power:float)->void:
 ...
 if fighter.current_state in [Fighter.State.IDLE,Fighter.State.MOVING]:
  # Distinct stance width/guard is expressed in bones only; collision stays fixed.
  rotate("Thigh.L",Vector3(0,0,.035*stance));rotate("Thigh.R",Vector3(0,0,-.035*stance))
  rotate("UpperArm.L",Vector3(0,0,.045*(style.guard-.7)));rotate("UpperArm.R",Vector3(0,0,-.045*(style.guard-.7)))
  var sway:=sin(time*(1.7*tempo));rotate("Spine",Vector3(0,.012*sway*twist,0));rotate("Head",Vector3(0,-.008*sway,0))
 if fighter.current_state==Fighter.State.STRIKING:
  var p:=clampf(fighter.state_timer/maxf(fighter.attack_total_time,.01),0,1);var impulse:=sin(PI*p)
  rotate("Hips",Vector3(0,.065*impulse*twist,0));rotate("Chest",Vector3(-.025*power*impulse,.07*impulse*twist,0))
  # Fast archetypes recoil sharply; planted archetypes keep the lower body quieter.
  rotate("Thigh.L",Vector3(-.025*impulse*(1-stance),0,0));rotate("Thigh.R",Vector3(.035*impulse*(1-stance),0,0))
...
func _dedicated_recovery() -> bool:
 # Only one layer may solve the recovering limbs. Preserve the supported palm
 # and fingers already evaluated by PairedContact at priority 30.
...
func _physics_process(delta:float)->void:
 ...
 _signature(style,mobility,power)
 if fighter.current_state==Fighter.State.STRIKING:
 ...
 if fighter.current_state==Fighter.State.GETTING_UP and not _dedicated_recovery():
  var p:=clampf(fighter.state_timer/.60,0,1);var hand_w:=1.-smoothstep(.48,.82,p);var knee_w:=smoothstep(.08,.30,p)*(1.-smoothstep(.72,.96,p));var hand_target:=fighter.global_position+fighter.global_basis.x*(-.30*body_scale)+fighter.global_basis.z*(.06*body_scale)+Vector3.UP*.025;var hand_pole:=hand_target-fighter.global_basis.z*.45+Vector3.UP*.22;_support_arm("L",hand_target,hand_pole,hand_w,"getup_hand");var foot_target:=fighter.global_position+fighter.global_basis.x*(.20*body_scale)-fighter.global_basis.z*(.24*body_scale)+Vector3.UP*.055;var knee_pole:=fighter.global_position-fighter.global_basis.z*(.42*body_scale)+Vector3.UP*.26;_support_leg("R",foot_target,knee_pole,knee_w,"getup_foot");rotate("Head",Vector3(-.16*(1-p),0,0))
 if stamina_ratio<.25 and fighter.current_state in [Fighter.State.IDLE,Fighter.State.MOVING]:var fatigue:=(.25-stamina_ratio)/.25;rotate("Chest",Vector3(.06*fatigue,0,0));rotate("Head",Vector3(-.035*fatigue,0,0))
