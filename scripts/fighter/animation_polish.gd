...
func setup(presentation_node:Node)->void:
 presentation=presentation_node;fighter=presentation.fighter;process_physics_priority=35
func reset()->void:
...
func _support_arm(side:String,target:Vector3,pole:Vector3,weight:float,kind:String)->void:
 ...
 if result.get("valid",false):
  result.merge({"kind":kind,"side":side,"target":target,"actual":IK.point(skeleton,bone("Hand."+side)),"weight":weight},true);diagnostics.append(result)
func _support_leg(side:String,target:Vector3,pole:Vector3,weight:float,kind:String)->void:
 ...
 if result.get("valid",false):
  result.merge({"kind":kind,"side":side,"target":target,"actual":IK.point(skeleton,bone("Foot."+side)),"weight":weight},true);diagnostics.append(result)
func _signature(style:Dictionary,power:float)->void:
 ...
 if fighter.current_state in [Fighter.State.IDLE,Fighter.State.MOVING]:
  rotate("Thigh.L",Vector3(0,0,.035*stance));rotate("Thigh.R",Vector3(0,0,-.035*stance));rotate("UpperArm.L",Vector3(0,0,.045*(style.guard-.7)));rotate("UpperArm.R",Vector3(0,0,-.045*(style.guard-.7)))
 ...
 if fighter.current_state==Fighter.State.STRIKING:
  ...
  rotate("Hips",Vector3(0,.065*impulse*twist,0));rotate("Chest",Vector3(-.025*power*impulse,.07*impulse*twist,0));rotate("Thigh.L",Vector3(-.025*impulse*(1-stance),0,0));rotate("Thigh.R",Vector3(.035*impulse*(1-stance),0,0))
...
func _physics_process(delta:float)->void:
 ...
 if fighter.current_state==Fighter.State.GETTING_UP:
  ...
  # Bring the authored body into a believable low support phase instead of asking
  # the arm solver to reach an impossible floor target from an upright torso.
  presentation.model.position.y-=.22*body_scale*hand_w
