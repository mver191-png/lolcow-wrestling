 ...
 if node is MeshInstance3D and node.mesh != null and node.skin != null:
  ...
  for i in range(mesh_node.mesh.get_surface_count()):
   ...
   if stride not in [4,8] or weights.size() != joints.size():
    ...
    continue
   _surfaces.append({"v":vertices,"j":joints,"w":weights,"stride":stride,"binds":binds,"bones":bone_ids,"material": str(mesh_node.get_active_material(i).resource_name) if mesh_node.get_active_material(i) else ""})
