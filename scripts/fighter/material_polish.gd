...
extends RefCounted
## Non-destructive runtime material tuning for imported original assets.
...
static func apply(root:Node)->int:
	...
	if root is MeshInstance3D and root.mesh:
		for surface in range(root.mesh.get_surface_count()):
			...
			var mat:=source.duplicate(true) as StandardMaterial3D
			...
			if name.contains("skin"):
				...
				mat.roughness=.76;mat.metallic=0.
			elif name.contains("boots"):
				mat.roughness=.40;mat.metallic=.02
			elif name.contains("gear") or name.contains("trim") or name.contains("wrap"):
				mat.roughness=.68 if name.contains("gear") else .52;mat.metallic=.01
